...
/*
 * $Id: MSTP-driver.h 79 2011-12-07 20:19:34Z xander $
 */
 ...
 *
 * MSTP-driver.h provides an API for the Mindsensors TouchPanel.
 *
 * Changelog:
 *  - 0.1 Initial	release.
 *
...
bool MSTPgetTouch(tSensors link, int &x, int &y, ubyte &buttons, ubyte addr) {
  ...
  if (!writeI2C(link, MSTP_I2CRequest, 3))
    return false;

  if (!readI2C(link, MSTP_I2CReply, 3))
    return false;
...
bool MSTPsendCmd(tSensors link, ubyte cmd, ubyte addr)
{
  ...
  return writeI2C(link, MSTP_I2CRequest, 1);
}
